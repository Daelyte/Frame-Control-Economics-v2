...
-- First, let's ensure we have the right tables for Frame Economics
-- This will work alongside your existing profiles/messages tables

-- Create users table for Frame Economics community (different from profiles)
...
-- Create stories table for Frame Economics community posts
...
-- Create comments table for story discussions
...
-- Create likes table for stories and comments
...
DROP POLICY IF EXISTS "Users can view non-banned profiles" ON public.users;
...
DROP POLICY IF EXISTS "Anyone can view approved stories" ON public.stories;
...
DROP POLICY IF EXISTS "Non-banned users can insert stories" ON public.stories;
...
DROP POLICY IF EXISTS "Users can update own non-flagged stories" ON public.stories;
...
DROP POLICY IF EXISTS "Non-banned users can insert comments" ON public.comments;
CREATE POLICY "Non-banned users can insert comments" ON public.comments 
  FOR INSERT WITH CHECK (
    auth.uid() = author_id 
    AND EXISTS (
      SELECT 1 FROM public.users 
...
-- Create triggers
DROP TRIGGER IF EXISTS story_likes_count_trigger ON public.likes;
...
DROP TRIGGER IF EXISTS comment_likes_count_trigger ON public.likes;
...
DROP TRIGGER IF EXISTS story_comments_count_trigger ON public.comments;
...
SELECT 
  ...
  'demo@frameeconomics.com',
  'Demo User',
  'demo'
WHERE NOT EXISTS (SELECT 1 FROM public.users WHERE id = '00000000-0000-0000-0000-000000000001'::uuid);

INSERT INTO public.stories (title, content, category, author_id) 
SELECT 
  'My First Frame Economics Success',
  'I applied Rule #1 (Patience Under Fire) in a difficult negotiation and it completely changed the outcome. The other person was trying to rush me into a decision, but I stayed calm and composed. This gave me the space to think clearly and respond strategically rather than reactively.',
  'success',
  '00000000-0000-0000-0000-000000000001'::uuid
WHERE NOT EXISTS (SELECT 1 FROM public.stories WHERE author_id = '00000000-0000-0000-0000-000000000001'::uuid);
