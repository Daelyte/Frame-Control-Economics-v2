// components/ambient/DragonBackdropWorking.tsx
// Server component (zero client JS). Guaranteed-visible dragon head + ambient wash.
// Default path = Game Icons "dragon head" (CC BY 3.0); see attribution below.
// To use CC0 instead, paste your CC0 <path d="..."/> into DRAGON_PATH_CC0 spot.

type Props = {
  hue?: number;         // 160–175 for jade/teal
  opacity?: number;     // 0.08–0.18 recommended
  useCC0?: boolean;     // true to switch to your CC0 path
};

export default function DragonBackdropWorking({
  hue = 170,
  opacity = 0.12,
  useCC0 = false,
}: Props) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden" aria-hidden>
      ...
      <svg viewBox="0 0 1600 900" className="absolute inset-0 w-full h-full" preserveAspectRatio="xMidYMid slice">
        ...
        <g transform="translate(180,60) scale(1.1)" opacity={opacity}>
          {/* If you pasted a CC0 path, render it here; else fall back to CC BY path */}
          <path
            d={/* useCC0 && DRAGON_PATH_CC0 ? DRAGON_PATH_CC0 : */ DRAGON_PATH_CC_BY}
            fill="none"
      ...
      {/*
        Attribution for the default inline path (required if you keep DRAGON_PATH_CC_BY):
        Dragon head by "Lorc" via Game-Icons.net, licensed CC BY 3.0.
        Source pages: game-icons dragon-head (Lorc), inline preview. See citations below.
      */}
    </div>
  );
}